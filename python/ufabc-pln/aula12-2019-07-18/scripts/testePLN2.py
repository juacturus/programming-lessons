...
if __name__ == '__main__':
    ...
    while True:
        ...
        if w in Vocabulary:
            ...
            for j in range(0, V): 
                if i!=j:
                    # calculando a distancia cosseno
                    dc = numpy.inner(PPMI[i,:], PPMI[j,:]) / (numpy.linalg.norm(PPMI[i,:])*numpy.linalg.norm(PPMI[j,:]))

                    if dc>=0.44:
                        txtGraph += '\n "{0}" -- "{1}"[label="{2:.4f}"]'. format(Vocabulary[i], Vocabulary[j], dc )
            txtGraph += "\n}"
            print(txtGraph)
        else:
